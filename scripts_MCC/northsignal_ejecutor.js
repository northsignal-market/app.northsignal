 ...
 *  Ejecuta acciones de las 4 cuentas: KAREDO, BHI, 360 y FRESH_MONKEE
 *  (vinculada al MCC el 12/9/2026).
 */
/**
 * NORTHSIGNAL · EJECUTOR v8
 * ----------------------------------------------------------------------------
 ...
 *
 * v8 (12 sep 2026):
 * - FRESH_MONKEE vinculada al MCC: el ejecutor ya puede aplicar sus acciones.
 *   La primera de FM conviene aprobarla en "Solo simular" para ver el circuito.
 * - Rama aplicar_etiqueta: figuraba ejecutable en el registro pero caia en
 *   "Tipo no soportado". Crea la etiqueta si no existe y la aplica a la campana.
 *
...
function _main() {
  ...
  for (var i = 0; i < pendientes.length; i++) {
    ...
    resumen.push('[' + a.account + '] ' + a.tipo + ' "' + (a.keyword || a.ad_id || a.campana || '') + '" → ' + r.estado + ': ' + r.resultado);
...
function ejecutar(a) {
  ...
  if (typeof AdsManagerApp === 'undefined') {
    ...
    if (!acct.hasNext()) return null;   // no vinculada al MCC: queda pendiente y visible en Ejecuciones
  ...
  if (a.tipo === 'aplicar_etiqueta') {
    // v8: figuraba ejecutable en capacidades_ejecucion pero no tenia rama y caia
    // en "Tipo no soportado". La etiqueta se crea si no existe y se aplica a la
    // campana. Es organizacion, no entrega: riesgo bajo, se revierte quitandola.
    var nombreEt = a.etiqueta;
    if (!nombreEt) return { estado: 'fallida', resultado: 'Falta la etiqueta' };
    if (a.modo === 'simular') return { estado: 'simulada', resultado: 'Etiquetaria ' + campaign.getName() + ' como "' + nombreEt + '"' };
    try {
      var escEt = String(nombreEt).replace(/"/g, '\\"');
      var hayEt = AdsApp.labels().withCondition('Name = "' + escEt + '"').get().hasNext();
      if (!hayEt) AdsApp.createLabel(nombreEt);
      campaign.applyLabel(nombreEt);
    } catch (e) { return { estado: 'fallida', resultado: 'No se pudo etiquetar: ' + e.message }; }
    return { estado: 'ejecutada', resultado: campaign.getName() + ' etiquetada como "' + nombreEt + '". PARA REVERTIR: quitarle la etiqueta.' };
  }
