...
function exportChangeEvents(ctx) {
  ...
  // RANGO CERRADO, LAS DOS PUNTAS. La API rechaza change_event con:
  //   "missing filters on change_event.change_date_time or is filtering on
  //    change_event.change_date_time with an infinite range"
  // Un >= sin techo ES un rango infinito para Google. La primera version de la v11 lo
  // dejo abierto y rompio la extraccion en las cuatro cuentas el 8 sep 21:20; a las 17:20
  // con la v10 todavia corria. Los tres agentes semanales lo encontraron esa misma noche.
  // Requisitos de la API para change_event, los tres a la vez:
  //   1. filtro sobre change_date_time con AMBOS extremos
  //   2. el rango no puede pasar de 30 dias
  //   3. LIMIT obligatorio
  // Se usan 28 dias y no 29 para dejar margen: change_date_time va en la zona horaria de
  // la cuenta y el borde de 30 dias se calcula alla, no en UTC.
  var tz30 = AdsApp.currentAccount().getTimeZone();
  var desde30 = Utilities.formatDate(new Date(new Date().getTime() - 28 * 86400000), tz30, 'yyyy-MM-dd');
  var hasta30 = Utilities.formatDate(new Date(), tz30, 'yyyy-MM-dd');
  var q =
    ...
    'AND change_event.change_date_time <= "' + hasta30 + ' 23:59:59" ' +
