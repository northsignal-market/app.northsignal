 ...
 *  12/9/2026; su copia interna se retira — no deben convivir).
 */
/**
 * NORTHSIGNAL — EXTRACCIÓN DIARIA  ·  v6
 * ============================================================================
 ...
 *
 * v6 (12 sep 2026):
 * - FRESH_MONKEE vinculada al MCC: este script la procesa como a las demás y la
 *   copia interna se desactiva.
 * - config_snapshot suma entity_type custom_conversion_goal. Hallazgo del ticket
 *   16: en BHI la definición de Conversions vive en el custom goal "interaccion"
 *   (3 acciones elegidas a dedo) y NINGUNA foto lo capturaba. Los flags por
 *   acción (primary_for_goal, include_in_conversions) NO gobiernan la métrica
 *   cuando hay custom goals, y ese hueco produjo un falso diagnóstico de
 *   definición mixta que costó un día de análisis.
 *
  ...
  ACCOUNTS: [
    {
      cid: '497-723-1137',
      label: 'FRESH_MONKEE',
      // 12 sep 2026: FM vinculada al MCC; este script la procesa como a las demas.
      // (Hasta hoy decia soloCuentaUnica: true y la corria una copia interna.)
...
function extractConfigSnapshot(ctx) {
  ...
  // --- Custom goals (12 sep 2026, hallazgo del ticket 16)
  // La definicion de Conversions puede vivir en un custom goal que elige acciones
  // a dedo e IGNORA los flags por accion. En BHI, el goal "interaccion" contiene
  // Envio de formulario + Asesoria Realizada + Cliente Activo, y por eso esas tres
  // cuentan aunque primary_for_goal e include_in_conversions digan false.
  // Sin esta foto, un cambio en el goal es invisible: ni change_event ni ninguna
  // otra tabla lo registra, y el proximo diagnostico vuelve a caer en el pozo.
  try {
    var nombresAccion = {};
    var rna = AdsApp.search('SELECT conversion_action.id, conversion_action.name FROM conversion_action WHERE conversion_action.status != "REMOVED"');
    while (rna.hasNext()) { var na = rna.next().conversionAction; nombresAccion[String(na.id)] = na.name; }
    var rcg = AdsApp.search('SELECT custom_conversion_goal.id, custom_conversion_goal.name, custom_conversion_goal.status, custom_conversion_goal.conversion_actions FROM custom_conversion_goal');
    while (rcg.hasNext()) {
      var cg = rcg.next().customConversionGoal;
      var acciones = (cg.conversionActions || []).map(function (rn) {
        var id = String(rn).split('/').pop();
        return nombresAccion[id] || id;
      }).sort();
      var cfgCg = { status: cg.status || null, acciones: acciones };
      ctx.rows.config_snapshot.push([
        ctx.label, hoy, 'custom_conversion_goal', cg.name || String(cg.id),
        JSON.stringify(cfgCg), hashOf(cfgCg)
      ]);
      n++;
    }
  } catch (e) { Logger.log('config_snapshot custom_conversion_goal: ' + e.message); }
 ...
 * INSTALACIÓN
 * ================================================================
 * 1. Reemplazar el código del script diario existente en el MCC.
 * 2. Vista previa y revisar el registro de ejecución.
 * 3. Programar TODOS LOS DÍAS a las 06:00.
 * 4. DESACTIVAR el script diario que vive adentro de Fresh Monkee.
 *
 ...
 *
 * Deberías ver unos 14 días por cuenta (las 4, FM incluida), con los más
 * recientes marcados como provisional y los de más de una semana como
 * consolidado.
 *
 * Y la foto nueva de custom goals:
 *
 *   select account, entity_name, config
 *   from config_snapshot
 *   where entity_type = 'custom_conversion_goal'
 *   order by snapshot_date desc;
 *
 * En BHI debe aparecer el goal "interaccion" con sus 3 acciones. El primer
 * día no detecta cambios: necesita dos snapshots para comparar.
 * ================================================================ */
