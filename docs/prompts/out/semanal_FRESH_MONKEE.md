# TAREA PROGRAMADA — FRESH MONKEE · v3 (Opus 5)

**Cowork › Scheduled › lunes 09:00 (Buenos Aires)** · Conectores: Supabase, Notion, Google Drive, Gmail
...
Al terminar, cerrá cada una: `select atender_nota(<id>, 'tarea_semanal_FRESH_MONKEE', '<qué encontraste, en dos frases>')`. Si una nota pide algo que no se puede responder con los datos que hay, cerrala igual diciendo qué faltaría. **Dejarla abierta sin decir nada es lo peor de los tres caminos.**

**¿Esta corrida tiene sentido?** `select corrida_redundante('FRESH_MONKEE')`. Si es lunes y `semana_cerrada_disponible` es false, la extracción semanal todavía no corrió (llega 07:00 UTC): registrás en `run_quality` con `que_fallo = 'semana cerrada no disponible'` y terminás. Si `redundante` es true, ya hubo corrida hoy sobre este período y no entró nada nuevo: una fila en `run_quality` y una línea de salida. Sin brief, sin correo, sin accionables. Si `hay_datos_nuevos` es true igual, seguís acotado a lo nuevo.

**La foto única.** `select get_estado_cuenta('FRESH_MONKEE')`. Si `accionables_abiertos` viene vacío, verificá contra Notion antes de creerlo: si Notion tiene abiertos y la foto no, el espejo no está sincronizado. Seguís leyendo de Notion y registrás un ticket.
...
- `coherencia_del_gasto`: el gasto de la semana y de la previa según la serie semanal. Si una cifra tuya no coincide, se calculó sobre otra base. **No hay delta de CPA de cuenta acá y no hay que inventarlo:** los objetivos están mezclados y el promedio no significa nada.
...
<el_mercado>
Desde el 13 de septiembre hay una capa que mira afuera de la cuenta: `select get_mercado_contexto('FRESH_MONKEE')`.
...
**El nombre de la entidad se resuelve, nunca se adivina.** `select entidad_existe('FRESH_MONKEE', '<tu accion jsonb>')`. Las negativas viven en `negatives`, no en `keywords`, y un objeto con comas es una lista, no un nombre.
...
**`completitud_de_cuenta('FRESH_MONKEE')`** dice qué le falta a la cuenta para operar bien. Si algo falta, es un ticket.
...
insert into predicciones (account, semana, metrica, valor_min, valor_max, probabilidad, razonamiento) values
  ('FRESH_MONKEE', '<lunes que empieza>', 'conversiones', <min>, <max>, 0.8, '<por qué ese rango, con los números que lo sostienen>'),
...
values ('<plataforma|benchmark|sector|metodo|regulacion>', '<título único>', '<qué dice, 2-4 oraciones con números>', '<URL>', '<google_oficial|agencia_con_datos|paper|foro_con_staff|medio_especializado>', <fecha o null>, '{FRESH_MONKEE}', '<qué hacer con esto, si algo>')
...
<reglas_de_dominio>
**Las reglas de esta cuenta viven en `cuentas.reglas_dominio` y salen por `get_contexto_sistema('FRESH_MONKEE')`.** Esa es la fuente: si algo de acá difiere, manda la base. Lo que sigue es lo que no se puede dejar de saber aunque la consulta falle.

**Estadística de una cadena.** El 77% de las campañas está bajo las 30 conversiones al mes.
...
Propiedades: `Brief` = "FRESH MONKEE · semana del <lunes> al <domingo>"; `Cliente`; `Semana`; `Brief anterior`; `Estado` = Generado; `Gasto`, `Conversiones`, `Delta CPA`, `Alertas ALTA`; `Dias provisionales`; `Titular`; `Handoff`; `Lecciones`.
...
`select idioma_reporte, nombre_contacto, encabezado_reporte, metricas_destacadas from cuentas where account = 'FRESH_MONKEE'`. El brief interno es siempre en castellano; solo esta sección cambia. Si `nombre_contacto` es null, no personalizás el saludo.
...
```sql
select latir('tarea_semanal_FRESH_MONKEE', true, null);
-- si se cortó a la mitad o no pudiste escribir el brief:
select latir('tarea_semanal_FRESH_MONKEE', false, 'qué falló, en una línea');
```
