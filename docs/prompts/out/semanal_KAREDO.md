# TAREA PROGRAMADA — KAREDO · v14 (Opus 5)
...
Los anuncios de esta cuenta van **en alemán**. El reporte al cliente sale en el idioma que diga `cuentas.idioma_reporte` — hoy `en` — y el brief interno siempre en castellano.
...
Al terminar, cerrá cada una: `select atender_nota(<id>, 'tarea_semanal_KAREDO', '<qué encontraste, en dos frases>')`. Si una nota pide algo que no se puede responder con los datos que hay, cerrala igual diciendo qué faltaría. **Dejarla abierta sin decir nada es lo peor de los tres caminos.**

**¿Esta corrida tiene sentido?** `select corrida_redundante('KAREDO')`. Si es lunes y `semana_cerrada_disponible` es false, la extracción semanal todavía no corrió (llega 07:00 UTC): registrás en `run_quality` con `que_fallo = 'semana cerrada no disponible'` y terminás. Si `redundante` es true, ya hubo corrida hoy sobre este período y no entró nada nuevo: una fila en `run_quality` y una línea de salida. Sin brief, sin correo, sin accionables. Si `hay_datos_nuevos` es true igual, seguís acotado a lo nuevo.

**La foto única.** `select get_estado_cuenta('KAREDO')`. Si `accionables_abiertos` viene vacío, verificá contra Notion antes de creerlo: si Notion tiene abiertos y la foto no, el espejo no está sincronizado. Seguís leyendo de Notion y registrás un ticket.
...
<el_mercado>
Desde el 13 de septiembre hay una capa que mira afuera de la cuenta: `select get_mercado_contexto('KAREDO')`.
...
<fuentes>
**Supabase** proyecto `djbwxgicosargfobsmqd`, filtrar siempre `account = 'KAREDO'`.
...
**Doc maestro:** `select get_doc_maestro('KAREDO')`. Ensamblado en vivo; Drive es export. **Notion:** ficha `3cf3b1f6-de28-81ea-ac05-c83c1fd3b219`, briefs `24cb7596-2bdf-460d-8d74-a8d042f55512`, accionables `4a8b366d-ed99-4098-a98d-949d76488032`.

**Tu paquete semanal es `select get_weekly_package('KAREDO')`.** **NO uses `get_weekly_package_cadena`**: ese es para cuentas multi-local. Si lo llamás igual devuelve vacío y perdés la corrida buscando por qué.
...
**El nombre de la entidad se resuelve, nunca se adivina.** `select entidad_existe('KAREDO', '<tu accion jsonb>')`. Las negativas viven en `negatives`, no en `keywords`, y un objeto con comas es una lista, no un nombre.
...
**`completitud_de_cuenta('KAREDO')`** dice qué le falta a la cuenta para operar bien. Si algo falta, es un ticket.
...
insert into predicciones (account, semana, metrica, valor_min, valor_max, probabilidad, razonamiento) values
  ('KAREDO', '<lunes que empieza>', 'conversiones', <min>, <max>, 0.8, '<por qué ese rango, con los números que lo sostienen>'),
...
<reglas_de_dominio>
**Las reglas de esta cuenta viven en `cuentas.reglas_dominio` y salen por `get_contexto_sistema('KAREDO')`.** Esa es la fuente: si algo de acá difiere, manda la base. Lo que sigue es lo que no se puede dejar de saber aunque la consulta falle.

**Antes de atribuir una caída a Alemania, verificá feriados y vacaciones escolares.** Fin de semana gasta la mitad: es normal.
...
Propiedades: `Brief` = "KAREDO · semana del <lunes> al <domingo>"; `Cliente`; `Semana`; `Brief anterior`; `Estado` = Generado; `Gasto`, `Conversiones`, `CPA`, `Delta CPA`, `Alertas ALTA`; `Dias provisionales`; `Titular`; `Handoff`; `Lecciones`.
...
`select idioma_reporte, nombre_contacto, encabezado_reporte, metricas_destacadas from cuentas where account = 'KAREDO'`. El brief interno es siempre en castellano; solo esta sección cambia. Si `nombre_contacto` es null, no personalizás el saludo.
