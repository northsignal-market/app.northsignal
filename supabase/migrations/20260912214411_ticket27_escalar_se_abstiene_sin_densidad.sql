-- Ticket 27: v_decision_estructural dictaminaba saturación citando escalones 15-20x
-- sobre 5-8 conversiones. La rama nueva se abstiene con densidad insuficiente (<10 conv
-- consolidadas en 28d), con el prefijo SIN DATOS que ya existe en el vocabulario de la vista.
-- Verificado lado a lado antes de aplicar: BHI (14 conv) y FRESH_MONKEE (301) conservan
-- su veredicto real; 360 (8) pasa a abstención honesta. "No se puede saber" > número inventado.
-- (Junto con esta migración se actualizó la relación #19 para aceptar la abstención como
-- cumplimiento, con doble prueba del mutante, y la relación #11 pasó de 8 a 15 días.)
create or replace view public.v_decision_estructural with (security_invoker='on') as
 WITH base AS (
         ...
         SELECT b_1.account,
            testeabilidad(b_1.conv_28d / 4.0, 4) AS t
           FROM base b_1
        )
 SELECT b.account,
    ...
    tst.t ->> 'veredicto'::text AS testeabilidad,
        ...
        END AS pausar,
        CASE
            WHEN COALESCE(b.conv_28d, 0::numeric) < 10::numeric THEN ((('SIN DATOS · solo '::text || round(COALESCE(b.conv_28d, 0::numeric))) || ' conversiones consolidadas en '::text) || COALESCE(b.dias_28d, 0::bigint)) || ' días: ninguna curva de saturación es creíble con esa densidad (mínimo 10 conv en 28d).'::text
