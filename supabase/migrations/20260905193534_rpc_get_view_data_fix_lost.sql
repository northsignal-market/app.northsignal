...
as $$
declare
  c_cost   text; c_clicks text; c_impr text; c_conv text;
  c_lost_b text; c_lost_r text;
...
begin
  ...
  case p_view
    ...
    when 'v_ngrams_sin_conversion' then
      c_cost:='costo_total'; c_clicks:='clics_totales';
    when 'v_fuzzy_negatives' then
      c_cost:='gasto_perdido'; c_clicks:='clicks';
  ...
  -- CPA y CTR se recalculan sobre las sumas, nunca se promedian por fila
  ...
  if c_lost_b is not null then
    v_sel_tot := v_sel_tot||', case when sum(coalesce('||quote_ident(c_lost_b)||',0)) > sum(coalesce('||quote_ident(c_lost_r)||',0))'
  ...
  execute 'select coalesce(json_agg(row_to_json(t)), ''[]''::json) from (select '||v_select||
          ...
          ' limit '||greatest(p_limit,1)||' offset '||greatest(p_offset,0)||') t' into v_data;
