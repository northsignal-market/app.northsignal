...
// Auth routes extracted to auth/routes.ts
  ...
  app.get("/r/:token", async (req, res) => {
    ...
    // Las claves son las que construye `get_reporte_datos`: cost, conversions, cpa,
    // ctr, clicks, impr_share. Esta página leía gasto/conversiones/clics/
    // cuota_impresiones —ninguna existe— así que al cliente le llegaba
    // "Spend —  Conversions —  CPA 45,39 €": solo salían las dos que por
    // casualidad se llaman igual en los dos lados. Es el mismo 7.1 del mensaje de
    // Slack, en la página que el cliente realmente abre.
    // La plantilla puede traer los nombres viejos, así que se traducen.
    const ALIAS: Record<string, string> = { gasto: 'cost', conversiones: 'conversions', clics: 'clicks', cuota_impresiones: 'impr_share' };
    const kpis = ((r.reporte_plantilla?.kpis || ['cost', 'conversions', 'cpa', 'clicks']) as string[]).map(k => ALIAS[k] || k);
    const kpiNombre: Record<string, string> = { cost: t.inv, conversions: t.conv, cpa: t.cpa, clicks: t.clics, ctr: t.ctr, impr_share: t.imprShare };
    const kpiHtml = kpis.map(k => `<div style="flex:1;min-width:120px;padding:14px 16px;background:#fff;border:1px solid #e5e7eb;border-radius:10px"><div style="font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:.04em">${kpiNombre[k] || k}</div><div style="font-size:22px;font-weight:600;color:#111827;margin:4px 0 2px">${fmt(r.metricas?.[k]?.actual, k === 'cost' || k === 'cpa' ? 'moneda' : k === 'ctr' || k === 'impr_share' ? 'pct' : 'num')}</div>${delta(k)}</div>`).join('');
...
async function coberturaDe(account: string, desde: string, hasta: string) {
  ...
  // El mismo contrato para los otros cuatro paneles de esta pantalla. Hasta el
  // 14 sep 2026 solo Integridad lo tenía, y los otros cuatro repetían el bug que
  // motivó escribirlo: leían nombres que la vista nunca tuvo y un `||` de la UI
  // tapaba el hueco con un default. El scorecard mostraba "15 / 15 pts" fijo,
  // Salud de datos decía "Sincronización al día" sobre una fila en ERROR, y cada
  // webhook decía "Último disparo: reciente" sobre un flujo que nunca recibió un
  // evento. Los nombres salen del DDL en 20260912182011_remote_schema.sql.
  ...
  app.get("/api/health/system", async (req, res) => {
    ...
    try {
      // El `error` de cada consulta se mira. Antes se destructuraba solo el de
      // integridad y las otras seis caían a `[]` ante una falla: una lista vacía
      // es indistinguible de "no hay nada que reportar", y en una pantalla de
      // salud ese es el peor empate posible — el panel se pinta tranquilo
      // justamente cuando no pudo preguntar.
  ...
  app.put("/api/health/run_quality/:id", async (req, res) => {
    ...
    try {
      ...
      if (/^\d{4}-\d{2}-\d{2}/.test(paramId)) {
        // `run_quality` tiene PK `id`, pero `v_run_scorecard` no la expone, asi que
        // la UI manda la fecha. `run_date` NO identifica una corrida: las cuatro
        // cuentas corren el mismo dia. Sin el filtro de cuenta, anotar la revision
        // humana de BHI la estampaba tambien en KAREDO, 360 y FRESH_MONKEE.
        // El radio de la escritura tiene que ser el radio de la evidencia.
...
Devolvé solo el texto del reporte, sin encabezado ni comentarios.`;
  ...
  app.post("/api/accionables/:id/aprobar-ejecutar", async (req, res) => {
    ...
    if (keywordsLote && tipo === 'pausar_keyword') {
      ...
      // Una negativa no es una keyword activa: se busca en la tabla de negativas.
      // Antes caia en resolverKeyword y devolvia "no encontre la keyword activa",
      // que para este verbo es siempre falso.
      const limpio = String(keyword).replace(/^[\[\"]+|[\]\"]+$/g, '').trim();
      let q = supabase.from('negatives').select('campaign, ad_group, match_type, negative_keyword')
        .eq('account', account).ilike('negative_keyword', `%${limpio}%`);
      if (campana) q = q.eq('campaign', campana);
      const { data: negs } = await q.order('run_ts', { ascending: false }).limit(5);
      if (!negs?.length) return res.status(422).json({ error: `No encontré la negativa "${keyword}" en ${account}. Puede que ya se haya quitado, o que esté escrita distinto. Verificá en Palabras clave negativas.` });
      const elegida = negs.find((n: any) => !body.nivel || (body.nivel === 'campana' ? !n.ad_group : !!n.ad_group)) || negs[0];
      camp = elegida.campaign; grp = elegida.ad_group || null; mt = elegida.match_type || mt;
  ...
  app.all("/api/cron/ghl-sondeo", async (req, res) => {
    ...
    // `pipelineStageId` dio 422 "property should not exist": el nombre del
    // parámetro es otro. En vez de adivinar de nuevo, se prueban las variantes
    // plausibles y gana la que conteste. Un 422 con el mensaje de GHL dice más
    // que mi memoria de su documentación.
    for (const { id, nombre } of idsDeDescarte) {
      ...
      let gano: any = null, intentos: string[] = [];
      ...
      pasos.push({
        ...
        intentos_fallidos: gano ? undefined : intentos,
    ...
    // Por estado, que es la otra puerta al descarte y no depende de la etapa.
    ...
    for (const ruta of [`/locations/${locationId}/customFields`, '/custom-fields/']) {
      ...
      pasos.push({
        ...
        campos: campos.map((c: any) => ({ id: c?.id, nombre: c?.name, clave: c?.fieldKey, tipo: c?.dataType })),
        candidatos_a_gclid: campos
          .filter((c: any) => /gclid|click|utm|campaign|keyword|wbraid|gbraid/i.test(`${c?.name} ${c?.fieldKey}`))
          .map((c: any) => ({ id: c?.id, nombre: c?.name, clave: c?.fieldKey })),
      });
      if (r.ok) break;
    }
