...
function createApp() {
  ...
  app2.get("/r/:token", async (req, res) => {
    ...
    const ALIAS = { gasto: "cost", conversiones: "conversions", clics: "clicks", cuota_impresiones: "impr_share" };
    const kpis = (r.reporte_plantilla?.kpis || ["cost", "conversions", "cpa", "clicks"]).map((k) => ALIAS[k] || k);
    const kpiNombre = { cost: t.inv, conversions: t.conv, cpa: t.cpa, clicks: t.clics, ctr: t.ctr, impr_share: t.imprShare };
    const kpiHtml = kpis.map((k) => `<div style="flex:1;min-width:120px;padding:14px 16px;background:#fff;border:1px solid #e5e7eb;border-radius:10px"><div style="font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:.04em">${kpiNombre[k] || k}</div><div style="font-size:22px;font-weight:600;color:#111827;margin:4px 0 2px">${fmt(r.metricas?.[k]?.actual, k === "cost" || k === "cpa" ? "moneda" : k === "ctr" || k === "impr_share" ? "pct" : "num")}</div>${delta2(k)}</div>`).join("");
